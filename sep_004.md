...
It is possible for there to be conflicts between the roles assigned to a
`Component` or `SequenceAnnotation` and the roles assigned to the underlying
child `ComponentDefinition` to which that `Component` or `SequenceAnnotation`
refers. Moreover, we discussed how there may arise situations where, up at the
level of a `Component` instance, users may wish to explicitly invalidate or
override roles declared down at the level of the child `ComponentDefinition`.
For example, whoever originally designed a part may have had no idea what it
really might eventually be used for. After discarding other options, we settled
on introducing an explicit property, `roleIntegration`, which specifies whether
roles on `Component` or `SequenceAnnotation` should (1) be added to (`merge`),
or (2) completely replace (`override`), any roles declared on the underlying
child `ComponentDefinition` to which they both refer.

As an alternative, it was first proposed to re-use `mapsTo.refinement` types.
These are
  * `useLocal` (akin to the proposed '`override`'' `roleIntegration`, meaning replace terms),
  * `useRemote` (ignore any terms declared on this instance),
  * `verifyIdentical` (see [SBOL] `MapsTo.refinement` for definition),
  * `merge` (akin to the proposed '`merge`'' `roleIntegration`).
...
implying a validation rule, as opposed to `merge` and `override`, which simply
name functions which, given multiple sets of roles, return a single set of
roles.
