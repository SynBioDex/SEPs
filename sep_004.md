...
roles on `Component` or `SequenceAnnotation` should (1) be added to
(`mergeRoles`), or (2) completely replace (`overrideRoles`), any roles declared
on the underlying child `ComponentDefinition` to which they both refer.
