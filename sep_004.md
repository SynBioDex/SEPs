...
Adding an optional `role` field to `Component` will allow users to describe the
...
Adding an optional `role` field to `SequenceAnnotation` will allow users to
...
We specify the contents of `SequenceAnnotation.role` and `Component.role` in
...
`SequenceAnnotation.roleRefinement` and `Component.roleRefinement` in analogy to
`MapsTo.refinement` ([SBOL] 7.7.3). Refinement rules specify how to resolve
potential conflicts between competing sets of roles.

### 2.1.0 Component.roles

The `Component.role` property comprises an OPTIONAL set of zero or more `role`
...
### 2.1.1 Component.roleRefinement

The `Component.roleRefinement` property is OPTIONAL and has a data type of URI.
Each `Component` instance MAY specify the relationship between its 'local' set
set of roles and the 'remote' set of roles on the child `ComponentDefinition`.

| Refinement URI                      | Description |
|-------------------------------------|-------------|
| http://sbols.org/v2#useRemote       | In the context of this `Component`, ignore any role(s) given for this `Component`. Instead use the set of zero or more roles given for the child `ComponentDefinition`. (*Deprecated* but included for congruence with `MapsTo.refinement`.) |
| http://sbols.org/v2#useLocal        | In the context of this `Component`, ignore any role(s) given for the child `ComponentDefinition`. Instead use only the set of zero or more roles given for this `Component`. |
| http://sbols.org/v2#verifyIdentical | Verify that the set of zero or more roles given for this `Component` is identical with the set of zero or more roles given for the child `ComponentDefinition`. Use that set. |
| http://sbols.org/v2#merge           | Use the union of the two sets: both the set of zero or more roles given for this `Component` as well as the set of zero or more roles given for the child `ComponentDefinition`. (*Default* value.) |

If no `roleRefinement` is given, then `http://sbols.org/v2#merge` is assumed.

It is RECOMMENDED to specify a set of `Component.role` elements only if the
refined set of roles differs from the 'remote' set of `role` elements belonging
to the child `ComponentDefinition`.

### 2.2.0 SequenceAnnotation.roles

(FIXME: make this the `roles` property as in SBOL. Also above.)

The `SequenceAnnotation.role` property comprises an OPTIONAL set of zero or more
...
### 2.2.1 SequenceAnnotation.roleRefinement

The `SequenceAnnotation.roleRefinement` property is OPTIONAL and has a data type
of URI. Each `SequenceAnnotation` instance MAY specify the relationship between
its 'local' set set of roles and the 'remote' set of roles computed for the
sibling `Component` (if any). The `remote` set of computed roles may include
`role` elements from the child `ComponentDefinition` if dictated by the value
of `Component.roleRefinement`; to determine the 'remote' `role` set for a
`SequenceAnnotation`, first compute the refined `role` set for the sibling `Component`
(if given) according to the refinement rules in section 2.1.1 above. If no
sibling `Component` is given, then the 'remote' set is assumed to be the empty
set.

| Refinement URI                      | Description |
|-------------------------------------|-------------|
| http://sbols.org/v2#useRemote       | In the context of this `SequenceAnnotation`, ignore any role(s) given for this `SequenceAnnotation`. Instead use the refined set of roles computed for the sibling `Component`. If no sibling `Component` is given, use the empty set. (*Deprecated* but included for congruence with `MapsTo.refinement`.) |
| http://sbols.org/v2#useLocal        | In the context of this `SequenceAnnotation`, ignore the refined set of roles computed for the optional sibling `Component`. Instead use only the set of zero or more roles given for this `SequenceAnnotation`. |
| http://sbols.org/v2#verifyIdentical | Verify that the set of zero or more roles given for this `SequenceAnnotation` is identical with the refined set of zero or more roles computed for the sibling `Component`. Use that set. |
| http://sbols.org/v2#merge           | Use the union of the two sets: both the set of zero or more roles given for this `SequenceAnnotation` as well as the refined set of zero or more roles computed for the optional sibling `Component`. (*Default* value.) |

If no `roleRefinement` is given, then `http://sbols.org/v2#merge`
is assumed.

It is RECOMMENDED to specify a set of `SequenceAnnotation.role` elements only if
the refined set of roles differs from the 'remote' set of `role` elements.

### 2.3 XML Serialization Details

For both proposed contextual `role` properties, a set of zero or more `<role>`
    ...
    <sbol:ComponentDefinition rdf:about="...">
        ...
        zero or more <sbol:component>
            ...
            <sbol:Component rdf:about="...">
                ...
                zero or one <sbol:roleRefinement rdf:resource="..."/> element
...
Gruenberg, and Chris Macklin (March 23, 2016).
