...
We propose adding optional `role` fields to both `SequenceAnnotation` and `Component`. This will allow users to specify the actual role(s) of a subcomponent or subsequence in the context of a design. In this document we refer to these two proposed `role` fields as "contextual roles."
