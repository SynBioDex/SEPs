...
integrated set of `roles` would differ from the 'remote' set of `roles` belonging
...
of `Component.roleIntegration`; to determine the 'remote' `role` set for a
...
the integrated set of SequenceAnnotation roles would differ from the 'remote' set of `roles`.
