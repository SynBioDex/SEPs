...
elements, then the `Component.role` set completely overrides all roles on its
...
contents of the two sets. (FIXME: refinement)

It is RECOMMENDED to specify a set of `Component.role` elements only if that set
of roles differs from the set of `role` elements already belonging to its child
`ComponentDefinition`.

If a Component's set of `role` elements contain multiple `role` URIs, then they MUST
identify non-conflicting terms (refer to examples in the [SBOL] spec).
(FIXME: remove MUST clauses that are impossible to validate)
...
merge the potentially conflicting contents of the three sets. (FIXME: refinement)

It is RECOMMENDED to specify a set of `SequenceAnnotation.role` elements only if that
set of roles differs from the set of `role` elements already belonging to its optional
...
`SequenceAnnotation.role` elements only if that set differs from the set of `role`
elements already belonging to the child `ComponentDefinition` (if any).

If a set of `SequenceAnnotation.role` elements contain multiple `role` URIs, then they
