...
|-------------------------------------|-------------|
| http://sbols.org/v2#useRemote       | In the context of this `Component`, ignore any role(s) given for this `Component`. Instead use the set of zero or more roles given for the child `ComponentDefinition`. (*Deprecated*, included for congruence with `MapsTo.refinement`.) |
