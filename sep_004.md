...
refined set of `roles` would differ from the 'remote' set of `roles` belonging
to the child `ComponentDefinition`.

### 2.2.0 SequenceAnnotation.roles

The `SequenceAnnotation.roles` property comprises an OPTIONAL set of zero or more
URIs describing the purpose or potential function of a given subsequence
...
`SequenceAnnotation`, first compute the refined set of roles for the sibling `Component`
...
| Refinement URI                      | Description |
|-------------------------------------|-------------|
| http://sbols.org/v2#useRemote       | In the context of this `SequenceAnnotation`, ignore any `roles` given for this `SequenceAnnotation`. Instead use the refined set of roles computed for the sibling `Component`. If no sibling `Component` is given, use the empty set. (*Deprecated* but included for congruence with `MapsTo.refinement`.) |
...
the refined set of SequenceAnnotation roles would differ from the 'remote' set of `roles`.
...
For both proposed contextual `roleRefinement` properties, either zero or one `<roleRefinement>` child element, if declared, MUST be contained within the parent element
in the XML serialized form.
