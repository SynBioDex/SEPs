...
its 'local' set of roles and the set of roles computed for the
sibling `Component` (if given). The integrated result set of computed roles may include
...
sibling `Component` is given, then the sibling set is assumed to be the empty
set.
...
the integrated result set of roles would differ from the integrated set of roles computed for the sibling Component.
