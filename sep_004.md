...
### 1.3 Argument for Both Component.roles and SequenceAnnotation.roles
...
`http://sbols.org/roleIntegration/v2#merge` is assumed.
...
If no `roleIntegration` is given, then
`http://sbols.org/v2/roleIntegration#merge` is assumed.
...
The roles given to a Component or SequenceAnnotation and the roles given to the
ComponentDefinition that they both refer to may end up being in conflict. Moreover,
it was discussed that there may be situations where users may, at the level of
Component instance, want to explicitely invalidate or override roles given at the
level of ComponentDefinition. The example being that whoever originally designed a part
may have no idea what it really would eventually be used for. After discarding other
options, we settled on introducing an explicit property, `roleIntegration` which
specifies whether roles of Component or SA should be (1) added to (`merge`) or (2)
should completely replace (`override`) any roles of the referred-to ComponentDefinition.

As an alternative, it was first proposed to re-use mapsTo.refinement types. These are
  * `useLocal` (=`override` or replace),
  * `useRemote` (ignore anything defined here),
  * `verifyIdentical`,
  * `merge` (=`merge`).

Both the use of `verifyIdentical` and `useRemote` where challenged as unnecessary
complications: The effect of `useRemote` is more obviously achieved by not defining
any new roles on Component; `verifyIdentical` was criticized for being an alltogether
different kind of beast (a validation rule).
