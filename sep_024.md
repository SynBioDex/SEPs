...
Numerical measurements SHOULD be specified as members of the `Measure`, `Unit`, and `Prefix` classes from the [Ontology of Units of Measure (OM)](http://www.ontology-of-units-of-measure.org/resource/om-2). As incorporated into the SBOL data model, `Measure` is a subclass of `Identified`, while `Unit` and `Prefix` are subclasses of `TopLevel` (Figure 1). 

![Measure, Unit, and Prefix class UML diagram](images/sep_024_measure_unit.png "Measure, Unit, and Prefix class UML diagram")
...
The `SingularUnit` class is used to describe a unit of measure that is not represented as 
...
The purpose of the `Prefix` class is to describe a parti
