...
- `inChildOf` must refer to a `ComponentInstance` pointed directly to by the parent of the `ComponentReference` (i.e., as a `subComponent` of `Component`, or an `inChildOf` of `ComponentReference`)
...
Everywhere that we can currently refer to a `ComponentInstance` (or SBOL2 subclass thereof), we adjust to refer to a `SubComponent`, thus allowing it to contain either instances or references.
...
Any SBOL 2 usage of `MapsTo` with a value other than `merge` can be converted to a `DesignConstraint`. As an optimization, non-useful `SubComponents` may be replaced with `ComponentReference` objects
...
These two changes have been moved to SEP 036.
