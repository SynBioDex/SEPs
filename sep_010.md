...
Add the following fields to `SequenceAnnotation`:
  ...
  * [1] `name` pointing to a short string literal for human consumption (required)
...
None. In the spirit of minimal change sets, this proposal could also be split up into three related SEPs (SequenceFeature, Merge SequenceAnnotation and Component, Rename SequenceAnnotation to Subpart).
