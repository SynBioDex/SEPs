...
Add the following *optional* field to `Component`:
  ...
  * [0..n] `location` pointing to a `Location` on the *parent* `ComponentDefinition` sequence; if missing, this indicates a part / sub-part relationship for which sequence details have not (yet) been determined. [SBOL 2.2]


### 2.2 Restrict `SequenceLocation` to sequence feature annotation

Remove the following fields from `SequenceAnnotation`:

  * `component` -- SequenceAnnotation is not any longer used for part - subpart relations [SBOL 3.0]
  * `roleIntegration` -- there is no sub-part with which `role` fields may be in conflict [SBOL 3.0]

Fields will be deprecated as of SBOL 2.2 and removed in SBOL 3.0.

The other fields are retained from the SBOL 2.1 `SequenceAnnotation` and `Identified`. For example:
  ...
  * [0..1] `name` corresponding to genbank name field (optional but recommended)
  * [0..1] `description` corresponds to genbank description field (optional)

Moreover, a *validation rule* is needed: SequenceAnnotation can only be used if an actual sequence record is specified for the parent ComponentDefinition.

### 2.3 Rename `SequenceAnnotation` to `SequenceFeature`

This backwards-incompatible change would be introduced with SBOL 3.0

### 2.4 Allow SequenceConstraints to point to `SequenceFeature` (`SequenceAnnotation`)

As of SBOL 2.2, SequenceConstraints can also point to `SequenceAnnotation`.
...
### 4.3 Backwards conversion of SBOL 3.x to 2.x records:
  ...
  * conversion of SequenceAnnotation / SequenceFeature
...
### 5.2 discussion at COMBINE
  ...
  * Renaming `SequenceAnnotation` to `SequenceFeature` was universally considered a good idea (for symmetry with genbank, bioinformatics practics and in order to avoid confusion with "Annotation" in SBML and elsewhere).
  * It was suggested to rename `SequenceConstraint` into `ComponentConstraint`.
  * It was suggested that `SequenceConstraint` should also be allowed to point to `SequenceAnnotation`. This would avoid construction of `Component` - `ComponentDefinition` chains for, e.g. mutations or other simple features that are not sub-parts but nevertheless restrict/orient the positioning of other Components.
