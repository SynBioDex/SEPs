...
This property references a `ComponentDefinition` that describes the actual, physical composition of a realized construct. In the case that the `Implementation` matches its original design, this may refer to the same `ComponentDefinition` which is also linked via PROV-O annotations. In case the actual physical composition does not match its specification, this property may reference a different `ComponentDefinition` which describes the actual, observed physical configuration. For example, this is useful when DNA sequencing results for a laboratory construct do not match its target sequence. 

#### 2.1.2 Implementation.moduleDefinition <a name="modDef"></a>

This property references a `ModuleDefinition` that describes the actual, observed functional behavior  of a realized construct. This behavior may or may not match the original design specification, and the referenced object may or may not be identical to the `ModuleDefinition` representing the design.
