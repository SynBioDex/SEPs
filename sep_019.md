...
* An `Activity` with a child Association of role "design" MAY generate either a `ModuleDefinition` or `ComponentDefinition` and MAY use a `Model` or other `ModuleDefinitions` or `ComponentDefinitions` as inputs via the `qualifiedUsage` property.
