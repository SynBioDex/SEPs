...
An `Implementation` represents a real, physical instance of a synthetic biological construct which may be associated with a laboratory sample. An `Implementation` describes the thing which was built during the Build phase of a D-B-T-L workflow. An `Implementation` may be linked back to its original design, either a `ModuleDefinition` or `ComponentDefinition` via PROV-O linkages as described in [Section 2.3](#dbtl). An `Implementation` may also link to a `ModuleDefinition` or `ComponentDefinition` which represents an `Implementation`'s actual structure and function after being realized in the laboratory. The `componentDefinition` and `moduleDefinition` properties of an `Implementation` are OPTIONAL, but if one of them is non-empty, then the other MUST be empty.
...
#### 2.1.1 Implementation.componentDefinition <a name="compDef"></a>

This property references a `ComponentDefinition` that describes the actual, physical composition of a realized construct. In the case that the `Implementation` matches its original design, this may refer to the same `ComponentDefinition` which is also linked via PROV-O annotations. In case the actual physical composition does not match its specification, this property may reference a different `ComponentDefinition` which describes the actual physical configuration. For example, this is useful when DNA sequencing results for a laboratory construct do not match its target sequence. 

#### 2.1.2 Implementation.moduleDefinition <a name="modDef"></a>

This property references a `ModuleDefinition` that describes the actual functional behavior of a realized construct. The `ModuleDefinition` for this behavior may or may not match the `ModuleDefinition` for the original design specification linked via PROV-O annotations.

### 2.2 Attachment <a name="attach"></a>

The `Attachment` class is a general container for data files, especially experimental data files. `Attachment` is a `TopLevel` object. `Attachment` has the following properties:

#### 2.2.1  Source <a name="source"></a>

The source is a link to the external file and is REQUIRED.

#### 2.2.2 Format <a name="format"></a>
...
### 2.4 Best Practices <a name="best_practices"></a>

#### 2.4.1 Usage Roles <a name="usage_roles"></a>
...
#### 2.4.2 Versioning versus Provenance Semantics <a name="provenance_semantics"></a>
