...
ComponentDefinition (red) with associated Component source locations (orange) and
Sequence Annotation locations (yellow) as described earlier. This insertion is also
specified by an in silico assembly Activity with Usages (orange) of the cargo and
backbone, plus a Plan (also orange) for their assembly. In this way, a tool that
does not fully support DNA assembly can still interpret the composition of a new
design in terms of non-modular pieces of previous designs, while a tool that does
support DNA assembly can expose a richer view of the biochemical mechanisms by which
this composition could occur.
