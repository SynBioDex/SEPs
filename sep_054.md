...
**Authors**           | Jacob Beal (jakebeal@ieee.org), Tom Mitchell, Bryan Bartley, Gonzalo Vidal, James Scott Brown, Helen Scott
**Editor**            | 
**Type**               | Data Model
...
Note also that [per W3C recommendation](https://www.w3.org/TR/swbp-vocab-pub/), content negotation may be used to determine whether to return SBOL (RDF) or browsable HTML.
...
To retrieve a static snapshot of package contents instead, the namespace can be changed to include the version (as described above). This MUST retrieve materials from the specific version of the package that have been rewritten to versioned identities as well, using the version of the package for its contents and the version at the time of publication for all references to dependencies.

This has the effect providing a static view of a snapshot of the package and its dependencies.

**TODO: rewriting of dissociated package dependencies**
...
If, at the time when version 1.3 was published, the fluorescent proteins package was on version 2.0, then the `SubComponent` link for GFP in the retrieved document would be `https://example.com/FluorescentProteins/2.0/GFP`.
