...
* `package` [ 1 ], URI: Package that is the dependecy or contains the dependency
...
* `object` [0,n], URI: If dependency is specific objects rather than a complete module or package, each instance of this property indicates one of those objects.
...
If all of the `Module` objects' namespaces share a prefix, then they form a well-defined `Package` with identity `[prefix]/package` and with `conversion=false` and `dissociated` not set. In this case, the `Package` will have no `member` values and its `hasDependency` values will be the union of the `hasDependency` values of its modules.
...
Any package catalog SHOULD contain conversion packages for common conversion packages, such as NCBI GenBank and the iGEM Parts Repository.
...
To this end, each `Package` in the catalog MUST have its `version` property set. The identity of the `Package` object in the catalog is then rewritten from `[namespace]/[displayId]` to `[namespace]/[version]/[displayId]`, with a `prov:wasDerivedFrom` link connecting it to its prior version.
...
The `Attachment` object points to a specific URIs from which a build artifact can be retrieved, that build artifact containing the `Package` and all of its contents for the identified release.
...
When there are multiple attachments in the same format, their contents MUST be identical. Identical contents can be tested for efficiently by comparing attachment hash values.

A packages may then be retrieved for use via its `Attachment` objects in the catalog to download a build artifact containing its contents, which may or may not require conversion after download.
...
A public shared catalog of published packages will be valuable to maintain. The working title for this collection the "SBOL index of packages", AKA "sip" (a deliberate reference to Python's [pip])

The catalog SHOULD NOT include arbitrary collections of designs without a clear engineering function. For example, a supplementary information file collecting all the genetic constructs in a scientific publication would not generally make a good sip package.
Likewise, forking of packages is discouraged: improvements to designs SHOULD result in a new version of a package rather than a new package.

A public catalog SHOULD be maintained in a version control system such as git. Such a version control system also implies standard methods such as merge requests can be used for publication of new packages and updated into the catalog. A local `sip` installation can then clone the catalog in order to being working with it, and can update the catalog by pulling updates at the beginning of each `sip` command execution.

A `sip` catalog MAY also be maintained in SynBioHub as a published collection. As published collections in SynBioHub not supposed to be modified, such a catalog may be downloaded once and assumed to remain current.
...
* `[root]/composites.nt`
* `[root]/regulatory/constitutive.nt` (includes iGEM material)
