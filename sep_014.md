...
**Authors**           | Bryan Bartley (bartleyba@sbolstandard.org), Ali Humphries, James McLaughlin, Goksel Misirli, Matthew Pocock, James Skelton, Angel Goni-Moreno, Anil Wipat
**Editor**            | <leave empty>
**Type**              | Data Model
**SBOL Version**      | 2.3
**Status**            | Draft
...
The `Test` class is simply a wrapper object that provides a link to an external data file, with analogy to the `Model` class. A `Test` class represents empirical data, while a `Model` represents a simulation. `Test` is subclassed from `Identified` .

![Test](images/sep_014_fig3.png "Test")

A `Test` object describes a single file. The URI of the file is indicated by the `source` property. A `Test` object is restricted to representing only one file because it may also includes metadata associated with that file. Currently, the metadata included are:
* `format`: An optional URI indicating the data format. Data formats may be inferred by the `source` field or explicitly stated with an ontology term. NuML (https://github.com/NuML/NuML)is a data encoding that we expect to support. Recommended URIs for this field are still needed.
* `size`: The byte size of the linked file. It is especially helpful to know this for very large files.
* `hash`: A file hash used by databases like SynBioHub
* `protocol`: A URI for a PROV-O `Plan` class, or perhaps a URI for a protocol document.
...
* Overloading existing fields with too many different ontologies is poor design
...
It is also necessary to link data. Since quality control data, ie, DNA sequencing data, describes the structural features and manufacturing outcomes, it is appropriate to attach these data to ComponentDefinitions. Other data containing characterization data of functional behavior are more appropriate to attach to ModuleDefinitions. Therefore, we conclude that a property called `tests` should be added to both `ComponentDefinition` and `ModuleDefinition` which contains URIs to corresponding `Test` objects. This ensures symmetry between structural and functional 
