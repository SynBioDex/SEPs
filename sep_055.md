...
The approaches recommended in this proposal may be applicable to other types of molecules besides DNA and methods of building, but considering of such is beyond the scope of this proposal.
...
This proposal does not propose any changes to the SBOL data model: it is intended to only be a set of practices to follow.
...
* **Part**: Design for a single contiguous linear DNA construct with a completely specified sequence.
  ...
  * **Composite Part**: A part that is designed as the composition of two or more other parts through an assembly. Note a composite part need not actually be produced by its designated assembly process: for example, the part might be synthesized directly, including the scars that would have been formed as part of assembly.
...
Any tools implementing these practices SHOULD NOT use equality testing, even for ontology terms with no children, because children may be added in future versions of the ontology.
...
A composite part is indicated by setting its `prov:wasGeneratedBy` property to a `prov:Activity` representing a RECOMMENDED assembly plan (see Assembly below).
Note that this implies that a composite part can be converted to a unitary part by removing its assembly plan (and vice versa, in cases where that is possible).
...
An assembled part within a composite part SHOULD be represented by the composite `Component` including a `feature` that is a `SubComponent` with an `instanceOf` proprerty linking to the part that has been included by assembly.
...
If the insertion site is at the beginning or end of the backbone, then the relationship between the part insert and backbone and two MAY be indicated by `Constraint` relations with the `meets` or `overlaps` restriction.
...
The assembly plan component SHOULD have a `SubComponent` for the composite part and for each assembled part that it includes.
...
To indicate the relationship between each part an that part in backbone, the assembly plan SHOULD contain a `Constraint` with a `type` value of `contains`, the part in backbone as the value of `subject` and the part as the value of `object`.
...
These part in backbone collection can be queried to find all of the backbones containing a given part P by searching the part in backbone collection for all `Components` that (recursively) contain a `SubComponent` whose `instanceOf` value is P.
...
Once the sequence has calculated, check for illegal restriction sites by scanning for all locations with sequences that match the recognition sites in the flanking been. Any such location that is not fully contained within a flanking sequence  means that the part in backbone is not compatible with the assembly.
